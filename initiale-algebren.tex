...
\note{\justifying
  ...
  Entsprechend sind viele (struturell-)korekursive Funktionen Beispiele für
  Anamorphismen -- also Morphismen, die uns die universelle Eigenschaft der
  terminalen Koalgebren schenken.
  \par
}
...
\note{\justifying
  ...
  sukzessive kontruieren kann. Trotzdem kann man seine Werte auch beobachten
  ...
  endlichen Listen ist, dass man seine Werte beobachten kann (ist eine solche
  Liste leer oder eine Cons-Zelle?). Trotzdem kann man aber auch seine Werte
  konstruieren (etwa an eine gegebene beliebig lange Liste vorne ein Element
  anfügen).
  \par
}
